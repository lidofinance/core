...
contract StakingRouter is AccessControlEnumerable, BeaconChainDepositor, Versioned {
  ...
  function unsafeSetExitedValidatorsCount(
    ...
    (
      ,
      ,
      /* uint156 targetLimitMode */ /* uint256 targetValidatorsCount */ uint256 stuckValidatorsCount,
      ,
      ,
  ...
  function onValidatorsCountsByNodeOperatorReportingFinished() external onlyRole(REPORT_EXITED_VALIDATORS_ROLE) {
    ...
    for (uint256 i; i < stakingModulesCount; ) {
      ...
      if (exitedValidatorsCount == stakingModule.exitedValidatorsCount) {
        ...
        try moduleContract.onExitedAndStuckValidatorsCountsUpdated() {} catch (bytes memory lowLevelRevertData) {
          /// @dev This check is required to prevent incorrect gas estimation of the method.
          ///      Without it, Ethereum nodes that use binary search for gas estimation may
  ...
  function getStakingModuleSummary(uint256 _stakingModuleId) public view returns (StakingModuleSummary memory summary) {
  ...
  function getStakingModuleMaxDepositsCount(
    ...
    (
      ,
      /* uint256 allocated */ uint256[] memory newDepositsAllocation,
    ...
    )
  {
    ...
    for (uint256 i; i < stakingModulesCount; ) {
      ...
      if (stakingModulesCache[i].activeValidatorsCount > 0) {
        ...
        stakingModuleValidatorsShare = ((stakingModulesCache[i].activeValidatorsCount * precisionPoints) /
          totalActiveValidators);

        recipients[rewardedStakingModulesCount] = address(stakingModulesCache[i].stakingModuleAddress);
        stakingModuleFee = uint96(
          (stakingModuleValidatorsShare * stakingModulesCache[i].stakingModuleFee) / TOTAL_BASIS_POINTS
        );
        ...
        totalFee += (uint96((stakingModuleValidatorsShare * stakingModulesCache[i].treasuryFee) / TOTAL_BASIS_POINTS) +
          stakingModuleFee);
  ...
  function deposit(uint256 _depositsCount, uint256 _stakingModuleId, bytes calldata _depositCalldata) external payable {
    ...
    if (_depositsCount > 0) {
      (bytes memory publicKeysBatch, bytes memory signaturesBatch) = IStakingModule(stakingModule.stakingModuleAddress)
