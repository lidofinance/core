...
*/
contract Lido3 is StETHPermit, AccessControlEnumerableUpgradeable {
    ...
    */
    function getInitializationBlock() public view returns (uint256) {
        return StorageSlot.getUint256Slot(INITIALIZATION_BLOCK_POSITION).value;
    }
    ...
    */
        ...
        reinitializer(_version)
    {
        ...
        StorageSlot.getAddressSlot(LIDO_LOCATOR_POSITION).value = _lidoLocator;
     ...
     */
    function finalizeUpgrade_v3(address _lidoLocator, address _eip712StETH) external reinitializer(_version) {
     ...
     */
    function setStakingLimit(uint256 _maxStakeLimit, uint256 _stakeLimitIncreasePerBlock) external onlyRole(STAKING_PAUSE_ROLE) {
     ...
     */
    function removeStakingLimit() external onlyRole(STAKING_PAUSE_ROLE) {
     ...
     */
    function setMaxExternalBalanceBP(uint256 _maxExternalBalanceBP) external onlyRole(STAKING_PAUSE_ROLE) {

        require(_maxExternalBalanceBP > 0 && _maxExternalBalanceBP <= TOTAL_BASIS_POINTS, "INVALID_MAX_EXTERNAL_BALANCE");

        StorageSlot.getUint256Slot(MAX_EXTERNAL_BALANCE_POSITION).value = _maxExternalBalanceBP;
    ...
    */
    ...
    function receiveELRewards() external payable {
        ...
        StorageSlot.getUint256Slot(TOTAL_EL_REWARDS_COLLECTED_POSITION).value = getTotalELRewardsCollected() + msg.value;
    ...
    */
     ...
     */
    function unsafeChangeDepositedValidators(uint256 _newDepositedValidators) external onlyRole(UNSAFE_CHANGE_DEPOSITED_VALIDATORS_ROLE) {

        StorageSlot.getUint256Slot(DEPOSITED_VALIDATORS_POSITION).value = _newDepositedValidators;
    ...
    */
    ...
    function getTotalELRewardsCollected() public view returns (uint256) {
        return StorageSlot.getUint256Slot(TOTAL_EL_REWARDS_COLLECTED_POSITION).value;
    }
    ...
    function getLidoLocator() public view returns (ILidoLocator) {
        return ILidoLocator(StorageSlot.getAddressSlot(LIDO_LOCATOR_POSITION).value);
    }
    ...
    */
    function getBeaconStat() external view returns (uint256 depositedValidators, uint256 beaconValidators, uint256 beaconBalance) {
        depositedValidators = StorageSlot.getUint256Slot(DEPOSITED_VALIDATORS_POSITION).value;
        beaconValidators = StorageSlot.getUint256Slot(CL_VALIDATORS_POSITION).value;
        beaconBalance = StorageSlot.getUint256Slot(CL_BALANCE_POSITION).value;
    }
    ...
    function deposit(
        ...
        if (depositsCount > 0) {
            ...
            StorageSlot.getUint256Slot(BUFFERED_ETHER_POSITION).value = _getBufferedEther() - depositsValue;
            emit Unbuffered(depositsValue);

            uint256 newDepositedValidators = StorageSlot.getUint256Slot(DEPOSITED_VALIDATORS_POSITION).value + depositsCount;
            StorageSlot.getUint256Slot(DEPOSITED_VALIDATORS_POSITION).value = newDepositedValidators;
    ...
    function mintExternalShares(address _receiver, uint256 _amountOfShares) external {
        ...
        uint256 newExternalBalance = StorageSlot.getUint256Slot(EXTERNAL_BALANCE_POSITION).value + stethAmount;
        ...
        StorageSlot.getUint256Slot(EXTERNAL_BALANCE_POSITION).value = newExternalBalance;
    ...
    function burnExternalShares(uint256 _amountOfShares) external {
        ...
        uint256 stethAmount = super.getPooledEthByShares(_amountOfShares);
        uint256 extBalance = StorageSlot.getUint256Slot(EXTERNAL_BALANCE_POSITION).value;

        if (extBalance < stethAmount) revert("EXT_BALANCE_TOO_SMALL");

        StorageSlot.getUint256Slot(EXTERNAL_BALANCE_POSITION).value = extBalance - stethAmount;
    ...
    function processClStateUpdate(
        ...
        StorageSlot.getUint256Slot(CL_VALIDATORS_POSITION).value = _reportClValidators;
        StorageSlot.getUint256Slot(CL_BALANCE_POSITION).value = _reportClBalance;
        StorageSlot.getUint256Slot(EXTERNAL_BALANCE_POSITION).value = _postExternalBalance;
    ...
    function collectRewardsAndProcessWithdrawals(
        ...
        uint256 postBufferedEther = _getBufferedEther()
            + _elRewardsToWithdraw // Collected from ELVault
            + _withdrawalsToWithdraw // Collected from WithdrawalVault
            - _etherToLockOnWithdrawalQueue; // Sent to WithdrawalQueue
    ...
    function _isMinter(address _sender) internal view override returns (bool) {
        return _sender == getLidoLocator().accounting();
    }

    /// @dev override isBurner from StETH to allow accounting to burn
    function _isBurner(address _sender) internal view override returns (bool) {
