...
/// @notice modified BLS wrapper to have validation method
...
library BLS {
    ...
    function unwrapG2(G2Point memory p) internal pure returns (bytes memory) {
        return abi.encodePacked(p.x_c0_a, p.x_c0_b, p.x_c1_a, p.x_c1_b, p.y_c0_a, p.y_c0_b, p.y_c1_a, p.y_c1_b);
    }

    uint constant WEI_PER_GWEI = 1e9;
    uint256 constant PUBLIC_KEY_LENGTH = 48;
    bytes constant NEG_G1_GENERATOR = "0xa7f1d3a731c711c6c3f3fec8f2b5a4d0bfd567c26cf1d52f40c5efb3c";
    ...
    function verifyDeposit(
        ...
        uint64 amount,
        bytes calldata signature // must be 96 bytes
    ) internal view {
        ...
        G2Point memory h = hashToG2(msgHash); // expected to be 96 bytes

        // The pairing precompile expects an input consisting of the concatenation of:
        //   (pubkey || h) and (NEG_G1_GENERATOR || signature)
        // where each G1 element is 48 bytes and each G2 element is 96 bytes.
        bytes memory input = abi.encodePacked(pubkey, unwrapG2(h), NEG_G1_GENERATOR, signature);
        ...
        if (output != 0) revert InvalidSignature();
    }
