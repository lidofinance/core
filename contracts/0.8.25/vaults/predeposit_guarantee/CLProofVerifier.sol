...
abstract contract CLProofVerifier {
    ...
    */
    // as provable parent node is first in the row at DEPTH 2
    ...
    GIndex public immutable GI_STATE_VIEW = pack((1 << STATE_ROOT_DEPTH) + STATE_ROOT_POSITION, STATE_ROOT_DEPTH);
    ...
    function _validatePubKeyWCProof(ValidatorWitness calldata _witness, bytes32 _withdrawalCredentials) internal view {
        ...
        GIndex gIndex = concat(GI_STATE_VIEW, concat(_getValidatorGI(_witness.validatorIndex), GI_PUBKEY_WC_PARENT));
