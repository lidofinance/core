...
npm run hardhat --network $NETWORK run --no-compile scripts/scratch/send-hardhat-mine.ts
...
NETWORK_STATE_FILE=deployed-local.json HARDHAT_FORKING_URL="${RPC_URL}" npm run hardhat --network hardhat run --no-compile scripts/scratch/scratch-acceptance-test.ts
