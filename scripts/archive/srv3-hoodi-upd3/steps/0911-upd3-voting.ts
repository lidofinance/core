...
export async function main() {
  ...
  // proxies
  const aoAddress = getAddress(Sk.accountingOracle, state);

  // new implementations
  const newAOImpl = state[Sk.accountingOracle].implementation.address;

  // get the currently active (old) DSM
  const voteDescription = process.env.VOTE_DESCRIPTION || "SRv3/CMv2 hoodi AO fix (update3)";
  ...
  const dgItems: VoteItem[] = [
    {
      ...
      call: { to: aoAddress, data: proxyIface.encodeFunctionData("proxy__upgradeTo", [newAOImpl]) },
    },
  ];
