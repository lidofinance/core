...
// https://github.com/ethereum/EIPs/blob/master/EIPS/eip-7251.md#configuration
export const EIP7251_ADDRESS = "0x0000BBdDc7CE488642fb579F8B00f3a590007251";
export const EIP7251_MIN_WITHDRAWAL_REQUEST_FEE = 1n;

export const deployEIP7251ConsolidationRequestContract = async (
  fee: bigint,
...
export const ensureEIP7251ConsolidationRequestContractPresent = async (): Promise<void> => {
  const code = await ethers.provider.getCode(EIP7251_ADDRESS);

  if (code === "0x") {
    log.warning(`EIP7251 consolidation request contract not found at ${EIP7251_ADDRESS}`);

    await deployEIP7251ConsolidationRequestContract(EIP7251_MIN_WITHDRAWAL_REQUEST_FEE);
    log.success("EIP7251 consolidation request contract is present");
  }
};
