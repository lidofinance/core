...
describe("Integration: Redeems reserve — skipped report", () => {
  ...
  it("skip → drain → deposit → recovery", async () => {
    ...
    expect(state4.reserve).to.equal(state3.reserve - redeemEther2);
    expect(state4.totalPooledEther).to.equal(state3.totalPooledEther - redeemEther2);
    expect(state4.totalShares).to.equal(state3.totalShares - redeemShares2);
