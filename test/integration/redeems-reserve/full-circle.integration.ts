...
describe("Integration: Redeems reserve — full circle", () => {
  ...
  it("happy path", async () => {
    ...
    expect(state3.reserve).to.equal(state2.reserve - redeemEther2);
    expect(await ethers.provider.getBalance(fix.address)).to.equal(redeemerBalanceBefore + redeemEther2);
    await assertReserveAllocationInvariant(lido);
  });
