...
describe("Integration: Redeems reserve — low TVL impact", () => {
  ...
  it("small TVL: redeem + WQ + rebase with two holders", async () => {
    ...
    await lido.connect(alice).submit(ZeroAddress, { value: ALICE_DEPOSIT });
    ...
    expect(state1.reserve).to.be.closeTo(0n, 10n);
    ...
    expect(state2.internalEther).to.equal(state1.internalEther + appliedRewards - ethLocked);
    ...
    expect(state0.reserveTarget - state2.reserveTarget).to.be.closeTo(
      RATIO_BP * internalEtherDrop / 10_000n, 10n
    );
