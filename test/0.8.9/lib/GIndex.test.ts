...
import { BigNumberish, BytesLike, concat, hexlify, randomBytes, ZeroHash, zeroPadValue } from "ethers";
...
describe("GIndex", () => {
  ...
  it("test_concat", async () => {
    ...
    expect(
      await gIndex.unwrap(
        await gIndex.concat(
          await gIndex.concat(
            await gIndex.concat(ROOT, await gIndex.pack(2, 1)),
            await gIndex.pack(5, 1)
          ),
    ...
    expect(
      await gIndex.unwrap(
        await gIndex.concat(
          await gIndex.concat(
            await gIndex.concat(ROOT, await gIndex.pack(2, 9)),
            await gIndex.pack(5, 1)
          ),
  ...
  it("test_concat_RevertsIfZeroGIndex", async () => {
    await expect(library.concat(ZERO, await gIndex.pack(1024, 1))).to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.concat(await gIndex.pack(1024, 1), ZERO)).to.be.revertedWithCustomError(library, "IndexOutOfRange");
  });

  it("test_concat_BigIndicesBorderCases", async () => {
    await expect(await library.concat(
      await gIndex.pack(2n ** 9n, 0),
      await gIndex.pack(2n ** 238n, 0)
    )).to.not.be.reverted;

    await expect(await library.concat(
      await gIndex.pack(2n ** 47n, 0),
      await gIndex.pack(2n ** 200n, 0)
    )).to.not.be.reverted;

    await expect(library.concat(
      await gIndex.pack(2n ** 199n, 0),
      await gIndex.pack(2n ** 48n, 0)
    )).to.not.be.reverted;
  });
  ...
  it("test_shr_AfterConcat", async () => {
    ...
    let gI = await gIndex.pack(1024, 4);
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5120, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5121, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 15)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5135, 4)));

    gI = await gIndex.pack(1031, 4);
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5127, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5128, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 8)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5135, 4)));

    gI = await gIndex.pack(2049, 4);
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10241, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10242, 4)));
    expect(await gIndex.unwrap(await gIndex.shr(await gIndex.concat(gIParent, gI), 14)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10255, 4)));
  });
  ...
  it("test_shr_OffTheWidth_AfterConcat", async () => {
    ...
    await expect(library.shr(await gIndex.concat(gIParent, ROOT), 1))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shr(await gIndex.concat(gIParent, await gIndex.pack(1024, 4)), 16))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shr(await gIndex.concat(gIParent, await gIndex.pack(1031, 4)), 9))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shr(await gIndex.concat(gIParent, await gIndex.pack(1023, 4)), 1))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
  });
  ...
  it("test_shl_AfterConcat", async () => {
    ...
    let gI = await gIndex.pack(1023, 4);
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(3071, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(3070, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 15)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(3056, 4)));

    gI = await gIndex.pack(1031, 4);
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5127, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5126, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 7)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(5120, 4)));
    ...
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 0)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10255, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 1)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10254, 4)));
    expect(await gIndex.unwrap(await gIndex.shl(await gIndex.concat(gIParent, gI), 15)))
      .to.equal(await gIndex.unwrap(await gIndex.pack(10240, 4)));
  });
  ...
  it("test_shl_OffTheWidth_AfterConcat", async () => {
    ...
    await expect(library.shl(await gIndex.concat(gIParent, ROOT), 1))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shl(await gIndex.concat(gIParent, await gIndex.pack(1024, 4)), 1))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shl(await gIndex.concat(gIParent, await gIndex.pack(1031, 4)), 9))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
    await expect(library.shl(await gIndex.concat(gIParent, await gIndex.pack(1023, 4)), 16))
      .to.be.revertedWithCustomError(library, "IndexOutOfRange");
  });
