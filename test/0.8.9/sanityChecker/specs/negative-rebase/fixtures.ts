...
import { NegativeRebaseFormulaCase, repeatReports, report } from "./lib";

export const negativeRebaseFormulaCases: NegativeRebaseFormulaCase[] = [
  {
  ...
  },
  {
    ...
    rationale: "The check is window-based; repeated tolerated reports can still fail once their sum crosses the limit.",
    reports: [
      report({
        label: "baseline report",
        preValidatorsBalance: ether("10000"),
  ...
  },
  {
    ...
    rationale: "At exactly 36 days from the old baseline timestamp, the baseline is still inside the inclusive window.",
    reports: [
      report({
