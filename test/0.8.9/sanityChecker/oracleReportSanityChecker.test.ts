...
describe("OracleReportSanityChecker.sol", () => {
  ...
  context("limits management", () => {
    ...
    it("setExitedValidatorEthAmountLimit: validates uint128 upper bound", async () => {
      await checker
        .connect(admin)
        .grantRole(await checker.EXITED_VALIDATOR_ETH_AMOUNT_LIMIT_MANAGER_ROLE(), manager.address);

      await expect(
        checker.connect(manager).setExitedValidatorEthAmountLimit((1n << 128n) + 1n),
  ...
  context("standalone sanity checks", () => {
    ...
    it("checkExitedEthAmountPerDay uses timeElapsed (seconds)", async () => {
      ...
      const exitedValidatorsCountForDailyExceededRevert =
        limitWithConsolidationInWei / exitedValidatorEthAmountLimit + 1n;
      const exitedPerDayForDailyExceededRevert =
        exitedValidatorsCountForDailyExceededRevert * exitedValidatorEthAmountLimit;
      ...
      const exitedPerDayForOneValidatorAndZeroTime = exitedValidatorEthAmountLimit * 86_400n;
  ...
  context("migrateBaselineSnapshot", () => {
    ...
    const deployMigratableChecker = async (contractVersion: bigint) => {
      const lido = await ethers.deployContract("Lido__MockForSanityChecker");
      await lido.mock__setContractVersion(contractVersion);
      await lido.mock__setBalanceStats(ether("100"), ether("7"), ether("3"));
      ...
      const migrationChecker = await ethers.deployContract("OracleReportSanityChecker", [
      ...
      return { migrationChecker, lido };
    };

    it("reverts if called by non-manager", async () => {
      const { migrationChecker } = await deployMigratableChecker(4n);
    ...
    it("reverts on unexpected Lido version", async () => {
      const { migrationChecker } = await deployMigratableChecker(3n);
