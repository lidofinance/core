...
contract MultiStakingVaultsTest is Test {
    ...
    function deployOperatorGrid() internal {
        ...
        operatorGridProxy.grantRole(operatorGridRegistryRole, rootAccount);

        operatorGridProxy.registerGroup(nodeOpAccount[0], groupShareLimit[0]);
        operatorGridProxy.registerGroup(nodeOpAccount[1], groupShareLimit[1]);
        ...
        operatorGridProxy.registerTiers(nodeOpAccount[0], tiersParamsGroup1);

        operatorGridProxy.registerTiers(nodeOpAccount[1], tiersParamsGroup2);
        vm.stopPrank();
    }
    ...
    function setUp() public {
        ...
        svSelectors[10] = msvHandler.updateVaultData.selector;
        svSelectors[11] = msvHandler.SVwithdraw.selector;
        svSelectors[12] = msvHandler.changeTier.selector;
    ...
    function invariant2_group_tier_liabilities_below_group_limit() external {
        for (uint256 i = 0; i < nodeOpAccount.length; i++) {
            ...
            assertLe(
                sumTierLiabilities,
                group.shareLimit,
                "Sum of tier liabilityShares in a group must not exceed group's shareLimit"
            );
        }
    }
    ...
    function invariant3_tier_liability_consistency() external {
        for (uint256 i = 0; i < operatorGridProxy.tiersCount(); i++) {
            ...
            assertEq(
                sumVaultLiabilities,
                tier.liabilityShares,
                "Sum of vaults' liabilityShares in a tier must equal tier's liabilityShares"
            );
        }
    }
    ...
    function invariant4_default_tier_liability_consistency() external {
        ...
        assertLe(
            sumVaultLiabilities,
            default_tier.shareLimit,
            "Sum of vaults' liabilityShares in the default tier must be less than or equal to the default tier's shareLimit"
        );
    }
    ...
    function invariant5_vault_connection_info() external {
        for (uint256 i = 0; i < stakingVaultProxies.length; i++) {
            ...
            assertEq(
                vc.reserveRatioBP,
                reserveRatioBP,
                "Vault's reserveRatioBP in connection must match OperatorGrid registered VaultInfo"
            );
            assertEq(
                vc.forcedRebalanceThresholdBP,
                forcedRebalanceThresholdBP,
                "Vault's forcedRebalanceThresholdBP in connection must match OperatorGrid registered VaultInfo"
            );
            assertEq(
                vc.infraFeeBP,
                infraFeeBP,
                "Vault's infraFeeBP in connection must match OperatorGrid registered VaultInfo"
            );
            assertEq(
                vc.liquidityFeeBP,
                liquidityFeeBP,
                "Vault's liquidityFeeBP in connection must match OperatorGrid registered VaultInfo"
            );
            assertEq(
                vc.reservationFeeBP,
                reservationFeeBP,
                "Vault's reservationFeeBP in connection must match OperatorGrid registered VaultInfo"
            );
        }
    }
}
