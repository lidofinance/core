 ...
 *
 */
//, Confirmable2Addresses {
contract OperatorGridMock is AccessControlEnumerableUpgradeable {
    /*
      Key concepts:
    ...
    function registerTiers(address _nodeOperator, TierParams[] calldata _tiers) external onlyRole(REGISTRY_ROLE) {
        if (_nodeOperator == address(0)) revert ZeroArgument("_nodeOperator");
    ...
    */
    ...
    function _validateParams(
        ...
        if (_infraFeeBP > MAX_FEE_BP) revert InfraFeeTooHigh(_tierId, _infraFeeBP, MAX_FEE_BP);

        if (_liquidityFeeBP > MAX_FEE_BP) revert LiquidityFeeTooHigh(_tierId, _liquidityFeeBP, MAX_FEE_BP);

        if (_reservationFeeBP > MAX_FEE_BP) revert ReservationFeeTooHigh(_tierId, _reservationFeeBP, MAX_FEE_BP);
    }
