...
contract StakingVaultsTest is Test {
        ...
        external
    {
        ...
        uint256 lockedAmount = vaultHubProxy.locked(address(stakingVaultProxy));
        (, uint256 obligationsFees) = vaultHubProxy.obligations(address(stakingVaultProxy));
        ...
        vaultNotPendingDisconnect
    {
        ...
        uint256 liabilityStETH = ILido(lido_addr).getPooledEthBySharesRoundUp(record.liabilityShares);
        if (liabilityStETH == 0) {
            return;
        }

        uint256 minHealthyTotalValue = Math256.ceilDiv(
            liabilityStETH * Constants.TOTAL_BASIS_POINTS,
            Constants.TOTAL_BASIS_POINTS - connection.forcedRebalanceThresholdBP
        );
