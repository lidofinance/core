  ...
  describe("verifyActiveValidatorsAfterExitRequest method", () => {
    ...
    it("accepts a valid proof and does not revert", async () => {
      ...
      const veboExitRequestTimestamp =
        GENESIS_TIME +
        (VALIDATOR_PROOF.beaconBlockHeader.slot - intervalInSlotsBetweenProvableBlockAndExitRequest) * SECONDS_PER_SLOT;
      ...
      const blockRootTimestamp = await updateBeaconBlockRoot(VALIDATOR_PROOF.beaconBlockHeaderRoot);
    ...
    it("accepts a valid historical proof and does not revert", async () => {
      ...
      const blockRootTimestamp = await updateBeaconBlockRoot(VALIDATOR_PROOF.futureBeaconBlockHeaderRoot);

      const tx = await validatorExitVerifier.verifyHistoricalActiveValidatorsAfterExitRequest(
        encodedExitRequests,
        {
          rootsTimestamp: blockRootTimestamp,
          header: VALIDATOR_PROOF.futureBeaconBlockHeader,
        },
        {
          header: VALIDATOR_PROOF.beaconBlockHeader,
          rootGIndex: VALIDATOR_PROOF.historicalSummariesGI,
          proof: VALIDATOR_PROOF.historicalRootProof,
        },
        [
          {
            exitRequestIndex: 0n,
    ...
    it("reverts if the validator proof is incorrect", async () => {
      ...
      const timestamp = await updateBeaconBlockRoot(VALIDATOR_PROOF.beaconBlockHeaderRoot);
