...
abstract contract OmnibusBase {
    ...
    function getNewVoteCallBytecode(string memory description) external view returns (bytes memory newVoteBytecode) {
        newVoteBytecode = CallsScriptBuilder
            .create(
                address(VOTING_CONTRACT),
                abi.encodeCall(VOTING_CONTRACT.newVote, (getEVMScript(), description, false, false))
            )
            ._result;
    }
    ...
    function isValidVoteScript(uint256 voteId) external view returns (bool) {
        (
            ...
            /*open*/ ,
            /*executed*/ ,
            /*startDate*/ /*snapshotBlock*/ /*supportRequired*/ /*minAcceptQuorum*/ /*yea*/ /*nay*/ /*votingPower*/ bytes memory script,
    ...
    function _forwardCall(
        ...
        return
            ScriptCall(
                forwarder,
                abi.encodeCall(IForwarder.forward, (CallsScriptBuilder.create(target, data).getResult()))
            );
    }
}
