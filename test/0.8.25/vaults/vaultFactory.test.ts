...
describe("VaultFactory.sol", () => {
  ...
  before(async () => {
    ...
    vaultFactory = await ethers.deployContract("VaultFactory", [locator, beacon, dashboard, admin], {
      from: deployer,
    });
  ...
  context("constructor", () => {
    ...
    it("reverts if `_lidoLocator` is zero address", async () => {
      await expect(ethers.deployContract("VaultFactory", [ZeroAddress, beacon, dashboard, admin], { from: deployer }))
    ...
    it("reverts if `_beacon` is zero address", async () => {
      await expect(
        ethers.deployContract("VaultFactory", [locator, ZeroAddress, dashboard, admin], {
          from: deployer,
        }),
      )
        .to.be.revertedWithCustomError(vaultFactory, "ZeroArgument")
    ...
    it("reverts if `_dashboard` is zero address", async () => {
      await expect(ethers.deployContract("VaultFactory", [locator, beacon, ZeroAddress, admin], { from: deployer }))
        .to.be.revertedWithCustomError(vaultFactory, "ZeroArgument")
