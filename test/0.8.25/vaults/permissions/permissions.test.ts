...
describe("Permissions", () => {
  ...
  context("initial state", () => {
    it("should have the correct roles", async () => {
      ...
      await checkSoleMember(disconnecter, await permissions.QUEUE_SELF_DISCONNECT_ROLE());
    });
  });
  ...
  context("grantRoles()", () => {
    it("mass-grants roles", async () => {
      const [
        ...
        permissions.QUEUE_SELF_DISCONNECT_ROLE(),
      ]);
  ...
  context("queueSelfDisconnect()", () => {
    it("queues a self-disconnect", async () => {
      await expect(permissions.connect(disconnecter).queueSelfDisconnect())
        .to.emit(vaultHub, "Mock__SelfDisconnectQueued")
        .withArgs(stakingVault);
    });

    it("reverts if the caller is not a member of the disconnect role", async () => {
      expect(await permissions.hasRole(await permissions.QUEUE_SELF_DISCONNECT_ROLE(), stranger)).to.be.false;

      await expect(permissions.connect(stranger).queueSelfDisconnect())
        .to.be.revertedWithCustomError(permissions, "AccessControlUnauthorizedAccount")
        .withArgs(stranger, await permissions.QUEUE_SELF_DISCONNECT_ROLE());
    });
  });
