    ...
    * forge-config: default.invariant.runs = 10000 
    ...
    function invariant_healthyVaultRemainsHealthy() public {
        ...
        if (wasHealthyBefore && !lastOpWasSettleFees) {
            assertTrue(
                isHealthyNow,
                "INVARIANT VIOLATED: Healthy vault became unhealthy after operation!"
            );
        }
     ...
     */
    function testFuzz_healthyVaultRemainsHealthy(
        uint96 operationType,
        uint96 param1,
        uint96 param2
    ) public {
    ...
    function incrementFundCount() external { ghost_fundCallCount++; }
    function incrementMintCount() external { ghost_mintCallCount++; }
    function incrementWithdrawCount() external { ghost_withdrawCallCount++; }
    function incrementRebalanceCount() external { ghost_rebalanceCallCount++; }
    function incrementForceRebalanceCount() external { ghost_forceRebalanceCallCount++; }
    function incrementBurnSharesCount() external { ghost_burnSharesCallCount++; }
    function incrementTransferAndBurnCount() external { ghost_transferAndBurnCallCount++; }
    function incrementUpdateConnectionCount() external { ghost_updateConnectionCallCount++; }
    function incrementPauseResumeCount() external { ghost_pauseResumeCallCount++; }
    function incrementSetLiabilityTargetCount() external { ghost_setLiabilityTargetCallCount++; }
    function incrementTriggerWithdrawalsCount() external { ghost_triggerWithdrawalsCallCount++; }
    function incrementSettleLidoFeesCount() external { ghost_settleLidoFeesCallCount++; }
